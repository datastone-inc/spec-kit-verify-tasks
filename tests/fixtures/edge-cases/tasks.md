...
- [X] EC004 Write unit tests for all validator functions in `tests/**/*.test.py`

<!-- EC005: Nested subtask structure -->
- [X] EC005 Set up the CI pipeline
  - [X] EC005.1 Add `.github/workflows/ci.yml` with test job
...
- [X] EC008 Refactor `src/auth.py` and `src/db.py` to share a common `BaseService` class defined in `src/base.py`
